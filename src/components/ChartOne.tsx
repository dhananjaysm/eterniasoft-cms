...
import { GET_SUBS_BY_DATERANGE } from "../graphql/query";
...
const ChartOne: React.FC = () => {
  ...
  // State for the chart data and selected granularity
  const [timeRange, setTimeRange] = useState({
    start: "2022-09-01",
    end: "2024-09-30",
  });
  const [granularity, setGranularity] = useState("month");
  const { data, loading, error } = useQuery(GET_SUBS_BY_DATERANGE, {
    variables: { startDate: timeRange.start, endDate: timeRange.end },
  });
  console.log(data);

  // State for the series data in the chart
  const [series, setSeries] = useState([
    { name: "Total Subs", data: [] },
    { name: "Total Users", data: [] },
  ]);

  // Update the series data when the GraphQL query returns new data
  useEffect(() => {
    if (data) {
      setSeries([
        { name: "Total Subs", data: data.getSubscriptionsByDateRange },
        { name: "Total Users", data: data.getSubscriptionsByDateRange },
      ]);
    }
  }, [data]);

  // Handlers for the granularity buttons
        ...
        <div className="flex flex-wrap w-full gap-3 sm:gap-5">
          <div className="flex min-w-47.5">
            ...
            <div className="w-full">
              <p className="font-semibold text-primary">Total Subs</p>
